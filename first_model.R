...
trainIndex <- createDataPartition(data$last_A, p = prob,
                                  list = FALSE,
                                  times = 1)
...
# print("Error RF :")
# print(prediction_error(dataTest$last_A, dataTest$predicted_rf_A))

result <- rbind(result, data.frame(
  size.train=prob, 
  error.glm.test=prediction_error(dataTest$last_A, dataTest$predicted_glm_A),
  error.glm.train=prediction_error(dataTrain$last_A, dataTrain$predicted_glm_A)
)
)
