...
for type_model in ['linearsvc', 'logistic']:
    tmp = models.get(type_model, {})
    models[type_model] = tmp
    for type_dataset in ['2', '3', 'all']:
        tmp = models[type_model].get(type_dataset, {})
        models[type_model][type_dataset] = tmp
        for model_letter in ['A','B','C','D','E','F','G']:
            model_filename = os.path.join(
                "model_%s" % type_model,
                "model_%s_data_%s_%s_not_centered.pkl" % (type_model, type_dataset, model_letter))
            print model_filename
            models[type_model][type_dataset][model_letter] = joblib.load(model_filename)
