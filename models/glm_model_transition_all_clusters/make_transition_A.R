...
for(debut in c(0,1,2)) {
  for(fin in c(0,1,2)) {
    ...
    rdata.filename <- file.path("DATA","TRANSITION",paste(paste("transition", debut, "vers", fin, "A", sep = "_"), "RData", sep="."))
    
    cat("Prc train", p, "debut =", debut, "fin =", fin, "\n")
    tmp <- get.train.test.transition.A(data, p=p, debut=debut, fin=fin)
