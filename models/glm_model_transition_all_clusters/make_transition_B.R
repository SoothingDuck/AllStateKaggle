...
for(debut in c(0,1)) {
  for(fin in c(0,1)) {
    ...
    rdata.filename <- file.path("DATA","TRANSITION",paste(paste("transition", debut, "vers", fin, "B", sep = "_"), "RData", sep="."))
    
    cat("Prc train", p, "debut =", debut, "fin =", fin, "\n")
    tmp <- get.train.test.transition.B(data, p=p, debut=debut, fin=fin)
