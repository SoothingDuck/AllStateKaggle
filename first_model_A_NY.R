...
write.csv(result, file.path("DATA","OUTPUT","result_model_A_NY.csv"))

# Entrainement final
print("Entrainement modele GLM 0 final")
model_0_final_A_NY <- glm(
  formula_0
  , family = binomial, data=dataTrainBase)

print("Entrainement modele GLM 1 final")
model_1_final_A_NY <- glm(
  formula_1
  , family = binomial, data=dataTrainBase)

print("Entrainement modele GLM 2 final")
model_2_final_A_NY <- glm(
  formula_2
  , family = binomial, data=dataTrainBase)

# Sauvegarde des modeles
save(model_0_final_A_NY, model_1_final_A_NY, model_2_final_A_NY, file=file.path("DATA","OUTPUT","first_model_A_NY.RData"))
